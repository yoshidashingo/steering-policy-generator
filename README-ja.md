...
2. Claude Codeを開き、ステアリングポリシーの生成を依頼:

```
コードレビューエージェント用のステアリングポリシーを生成してください。
```

例:

```
提案書作成エージェント用のステアリングポリシーを生成してください。
セキュリティ監査エージェント用のステアリングポリシーを生成してください。
```

3. ジェネレーターが4フェーズのワークフローに沿って対話的にガイドします:
...
4. 各ステージで確認と承認を行ってから次に進みます。明示的な承認なしにジェネレーターが先に進むことはありません。
